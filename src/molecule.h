...
struct SurfaceParams {
    ...
    std::string type = "tses";  ///< Surface type: "tses" (triangulated solvent excluded surface) or
                                ///< "ases" (analytical surface) (default: "tses")
    std::string program =
        "msms";  ///< Surface generation program: "msms" or "fibonacci" (default: "msms")
    ...
    bool include_normals = false;  ///< Whether to include vertex normals in output (default: false
                                   ///< for better PyMOL compatibility)
...
class Molecule {
  public:
     ...
     */
    std::unordered_map<int, int> generateAdvancedConformers(
        const ConformerParams& params = ConformerParams());
     ...
     */
    void compute(const std::string& property_name, unsigned int conformer_id = 0,
     ...
     *   auto potentials = molecule.getProperty<std::vector<double>>("esp",
    ...
    const T& getProperty(const std::string& property_name, const std::string& key,
        ...
        if (cache_it == surface_property_cache_.end()) {
            throw std::runtime_error(
                "Property '" + property_name + "' not computed for conformer " +
        ...
        if (key_it == cache_it->second.end()) {
            throw std::runtime_error("Property key '" + key + "' not found for property '" +
                                     property_name + "' in conformer " +
                                     std::to_string(conformer_id));
        }

        try {
            return std::any_cast<const T&>(key_it->second);
        } catch (const std::bad_any_cast& e) {
            throw std::runtime_error("Property key '" + key + "' has wrong type for property '" +
                                     property_name + "'");
        }
    }
     ...
     */
    int createSurface(unsigned int conformer_id = 0, unsigned int num_vertices = 1000,
                      double radius = 1.2, double density = 3.0, double hdensity = 3.0,
                      std::string type = "tses", std::string program = "msms",
                      std::string sample_method = "full", bool addH = true);
     ...
     * @throws std::runtime_error if surface is not found
     */
    const std::map<int, std::pair<std::string, std::vector<double>>>& getSurfaceVertices(
     ...
     */
    Eigen::Isometry3d align_to_molecule(Molecule& target_mol, unsigned int target_conformer_id = 0,
                                        unsigned int source_conformer_id = 0);
     ...
     */
    AlignmentResult align_to_molecule_detailed(Molecule& target_mol,
                                               unsigned int target_conformer_id = 0,
                                               unsigned int source_conformer_id = 0);
     ...
     */
    void saveConformerSDF(unsigned int conformer_id, const std::string& filename,
                          bool include_properties = true) const;
     ...
     */
    void saveSurfacePointCloud(unsigned int conformer_id, const std::string& filename,
                               bool include_normals = true, bool include_colors = false,
     ...
     * @throws std::runtime_error if surface doesn't exist or file cannot be written
     */
    void saveSurfaceMesh(unsigned int conformer_id, const std::string& filename,
                         bool include_normals = true, bool include_colors = false,
                         const std::vector<std::string>& properties = {}) const;
