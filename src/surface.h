...
class Surface {
  public:
     ...
     * @param type Surface type - "tses" for triangulated solvent excluded surface, "ases" for
     * analytical (default: "tses")
     * @param program Surface generation program to use - "msms" or "fibonacci" (default: "msms")
     * @param sample_method Vertex sampling method - "random", "fps" (farthest point sampling), or
     * "full" (default: "full")
     *
     * @throws std::invalid_argument if num_vertices <= 4, radius <= 0, density <= 0, or hdensity <=
     * 0
     ...
     */
    Surface(const Molecule& molecule, int conformer_id, int num_vertices, double radius = 1.2,
            double density = 3.0, double hdensity = 3.0, std::string type = "tses",
            std::string program = "msms", std::string sample_method = "full");
     ...
     * - Otherwise: generates 3D coordinates from molecule, creates xyzr file, then generates
     * surface
     *
     ...
     */
    void generateFibonacciSurface(
        const spalt::FibonacciSurfaceParams& fibonacci_params = spalt::FibonacciSurfaceParams());
     ...
     */
    std::map<int, std::pair<std::string, std::vector<double>>> select_n_vertices(
     ...
     */
    void savePointCloud(
        const std::string& filename, bool include_normals = true, bool include_colors = false,
        const std::vector<std::string>& properties = {},
        const std::unordered_map<std::string, std::any>* property_cache = nullptr) const;
     ...
     * @throws std::runtime_error if file cannot be written
     */
    void saveMesh(const std::string& filename, bool include_normals = true,
                  bool include_colors = false, const std::vector<std::string>& properties = {},
                  const std::unordered_map<std::string, std::any>* property_cache = nullptr) const;
  ...
  private:
     ...
     */
    std::tuple<int, int, int> getVertexColor(
     ...
     * @return RGB color as tuple (red, green, blue) in range [0, 255]
     */
    std::tuple<int, int, int> propertyValueToColor(
        const std::string& property_name, double value,
        const std::unordered_map<std::string, std::any>* property_cache) const;
     ...
     */
    std::map<int, std::pair<std::string, std::vector<double>>> select_n_vertices_random(
        size_t n_vertices);
     ...
     * @return Map of selected vertices
     */
    std::map<int, std::pair<std::string, std::vector<double>>> select_n_vertices_fps(
        size_t n_vertices);
