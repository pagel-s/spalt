...
struct SurfaceESPProperty : ISurfaceProperty {
     ...
     */
    [[deprecated(
        "Use constructor parameter instead: SurfaceESPProperty(ChargeMethod::RDKIT)")]] void
    compute(const Surface& s, std::unordered_map<std::string, std::any>& cache,
            ChargeMethod method);
     ...
     */
    static std::vector<double> get_esp_charges(
     ...
     */
    std::vector<double> compute_esp_potentials(const Surface& surface,
     ...
     * @throws std::bad_any_cast if cache contains wrong data type
     */
    static std::vector<double> get_esp_potentials(
        const std::unordered_map<std::string, std::any>& cache);
     ...
     */
    ChargeMethod getChargeMethod() const {
        return charge_method_;
    }
  ...
  private:
     ...
     */
    std::vector<double> compute_rdkit_charges(const Surface& s,
                                              std::unordered_map<std::string, std::any>& cache);
};