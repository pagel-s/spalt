...
class ConformerGenerator {
  public:
     ...
     */
    std::unordered_map<int, int> generateConformers(std::shared_ptr<RDKit::ROMol> mol,
    ...
    generateConformersFromSmiles(const std::string& smiles,
                                 const ConformerParams* params = nullptr);

    /**
     * @brief Get current parameters
     */
    const ConformerParams& getParams() const {
        return params_;
    }
     ...
     */
    void setParams(const ConformerParams& params) {
        params_ = params;
    }
  ...
  private:
     ...
     */
    void sampleOptimizedConformers(std::shared_ptr<RDKit::ROMol> mol, int num_confs,
                                   const ConformerParams& params);

    /**
     * @brief Sample conformers using ETKDG method (1/6 of total)
     */
    void sampleETKDGConformers(std::shared_ptr<RDKit::ROMol> mol, int num_confs,
                               const ConformerParams& params);

    /**
     * @brief Sample conformers with uniform random coordinates (1/6 of total)
     */
    void sampleUniformConformers(std::shared_ptr<RDKit::ROMol> mol, int num_confs,
                                 const ConformerParams& params);
     ...
     */
    std::unordered_map<int, int> clusterConformers(std::shared_ptr<RDKit::ROMol> mol,
                                                   int num_clusters);
     ...
     * @return Map of cluster_id -> conformer_id
     */
    std::unordered_map<int, int> selectClusterConformers(const std::vector<int>& cluster_labels,
                                                         int num_clusters);
