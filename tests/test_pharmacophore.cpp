...
TEST_F(PharmacophoreTest, ComputePharmacophoreAromatic) {
    ...
    auto vals = mol_benzene->getProperty<std::vector<double>>("pharma_aromatic", "pharma_aromatic_values", 0);
    EXPECT_FALSE(vals.empty());
    
    // Benzene should have some positive aromatic values
    bool has_aromatic = false;
    for (double v : vals) {
        if (v > 0.0) has_aromatic = true;
    }
...
TEST_F(PharmacophoreTest, ComputePharmacophoreDonor) {
    ...
    auto vals = mol_amine->getProperty<std::vector<double>>("pharma_donor", "pharma_donor_values", 0);
    EXPECT_FALSE(vals.empty());
}

TEST_F(PharmacophoreTest, ComputePharmacophoreAcceptor) {
    mol_ketone->compute("pharma_acceptor", 0);
    auto vals = mol_ketone->getProperty<std::vector<double>>("pharma_acceptor", "pharma_acceptor_values", 0);
    EXPECT_FALSE(vals.empty());
}
