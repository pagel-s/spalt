...
TEST_F(NormalsUsageTest, SurfaceVerticesContainNormals) {
    ...
    for (const auto& [index, vertex_data] : surface.vertices) {
        ...
        EXPECT_EQ(coords_normals.size(), 6)
            << "Vertex " << index << " should have 6 elements (coordinates + normals)";
        ...
        if (coords_normals.size() >= 6) {
            ...
            EXPECT_LT(abs(normal_magnitude - 1.0), 0.1)
                << "Normal vector should be approximately unit length";
        }
    }
}
...
TEST_F(NormalsUsageTest, Open3DPointCloudPreservesNormals) {
    ...
    for (size_t i = 0; i < source_cloud->normals_.size(); ++i) {
        ...
        EXPECT_LT(abs(magnitude - 1.0), 0.1)
            << "Source normal " << i << " should be approximately unit length";
    }
    ...
    for (size_t i = 0; i < target_cloud->normals_.size(); ++i) {
        ...
        EXPECT_LT(abs(magnitude - 1.0), 0.1)
            << "Target normal " << i << " should be approximately unit length";
    }
}
...
TEST_F(NormalsUsageTest, SurfaceExportPreservesNormals) {
    ...
    while (std::getline(file, line)) {
        if (line.find("property float nx") != std::string::npos)
            found_nx = true;
        if (line.find("property float ny") != std::string::npos)
            found_ny = true;
        if (line.find("property float nz") != std::string::npos)
            found_nz = true;
...
TEST_F(NormalsUsageTest, FibonacciSurfaceGeneratesNormals) {
    ...
    Surface surface(*test_molecule, 0, params.num_vertices, params.radius, 3.0, 3.0, "tses",
                    "fibonacci", "full");
    ...
    for (const auto& [index, vertex_data] : surface.vertices) {
        const auto& coords_normals = vertex_data.second;
        EXPECT_EQ(coords_normals.size(), 6)
            << "Fibonacci surface vertex " << index << " should have 6 elements";
        ...
        if (coords_normals.size() >= 6) {
            ...
            EXPECT_GT(normal_magnitude, 0.1)
                << "Fibonacci surface normal should have reasonable magnitude";
            EXPECT_LT(abs(normal_magnitude - 1.0), 0.2)
                << "Fibonacci surface normal should be approximately unit length";
        }
    }
}
